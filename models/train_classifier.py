...
from utils.utils import (
    tokenize,
    VerbCountExtractor,
    NegationCountExtractor,
    EmotionWordCountExtractor,
    PunctuationCountExtractor,
    TextLengthExtractor,
    CapitalizationCountExtractor,
        ...
        for i in range(number_of_categories):
            ...
            class_report = pd.DataFrame(
                classification_report(
                    Y_test.iloc[:, i], y_pred[:, i], labels=[0, 1], output_dict=True
                )
            ).transpose()
...
def main():
    if len(sys.argv) == 3:
        ...
        rp_path = "./reports/"
        evaluate_model(
            model, X_test, Y_test, category_names, f"{rp_path}classification_report.md"
        )
